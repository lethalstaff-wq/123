...
def s_platforms() -> str:
    ...
    ba = R.r("r__bought_accounts")
    if ba.get("key_numbers"):
        prices = "".join(f"<li>{e(str(x)[:190])}</li>" for x in ba["key_numbers"][:5])
        body += (f'<h3>Рынок покупных аккаунтов: цифры</h3>'
                 f'<ul class="small">{prices}</ul>'
                 f'<div class="callout"><h4>Гарантия продавца не действует в твоём сценарии</h4>'
                 f'<p>Гарантия — 48 часов, и по правилам самих продавцов она аннулируется, если заходить '
                 f'с домашнего IP, через VPN, прокси или виртуалку, либо держать несколько аккаунтов на '
                 f'одном устройстве или прокси. То есть при любой реальной схеме сетки гарантии нет. '
                 f'Возвраты только в Perfect Money, Advcash, WebMoney и LTC — чарджбэка не существует.</p>'
                 f'<p>Масштаб отлова на стороне площадок: TikTok заявляет о предотвращении более '
                 f'2 млрд созданий спам-аккаунтов за год и удалении 143 834 113 фейковых аккаунтов за '
                 f'IV квартал 2025 при доле проактивного обнаружения 99.1%, из них 93.4% в первые сутки. '
                 f'Meta заблокировала около 1 млрд фейковых аккаунтов на Facebook за I квартал 2025.</p>'
                 f'<p><b>Отдельный технический факт:</b> TikTok заявляет о работе с мобильными операторами '
                 f'по блокировке SIM-карт, которые используются только для создания аккаунтов. Пулы номеров '
                 f'SMS-активации помечены на уровне оператора ещё до того, как ты на них зарегистрируешься.</p></div>'
                 f'<p class="small">Существенная деталь: TikTok прямо разрешает иметь несколько аккаунтов. '
                 f'Запрещены автоматизация ведения множества аккаунтов и обман, а не множественность сама '
                 f'по себе. Вывод ресерча: при бюджете 50–200 долларов в месяц сетка из двадцати купленных '
                 f'каналов на гео экономически проигрывает — не из-за цены аккаунта, а из-за того, что бан '
                 f'накрывает сеть целиком, включая новые аккаунты.</p>')
